...
/* MACRO, CONSTANTES */ /* macro, constant */

/* precision des differentes fonction de calcul */ /* precision of the different calculation functions */

#define MAX_OPEN_NODES 15000

#define SCORE_TYPE double
/* permet de definir la core_table en char pour les grosses BDs */ /*allows to define the core_table in char for large databases*/
#define CORETABLE_TYPE float
/* #define CORETABLE_TYPE unsigned char */

#define MAX_NUMBER_OF_ATTRIBUTES 300000

/* pour les variables symboliques */ /* for the symbolic variables */

#define MAX_NUMBER_OF_SYMBOLIC_VALUES 1024
#define MAX_NUMBER_OF_SYMBOLIC_VALUES_DIV_32 32 /* C'est à dire maximum 128 valeurs */ /* i.e max 128 values */
...
/* redefinition du malloc (pour l'interface matlab) */ /* redefinition of the malloc for the matlab interface */
#define MyMalloc malloc
#define MyFree free

/* definition de types */ /* definition of types */
...
/* prototype de fonctions */ /* prototype of functions */

/* fonction qui calcule des stats pour le vecteur et qui place le resultat dans table_score */ /* function which calculates stats for the vector and which places the result in table_score */
void (*summarize_vector)(int *vector, int start, int end);
/* fonction qui cree un vecteur de prediction */ /* function tat creates a vector of prediction */
int (*make_leaf_prediction)();
/* fonction qui renvoie true si on doit arreter la construction */ /* function that returns true if we must stop the construction */ 
int (*stop_splitting_criterio)();
/* fonction qui renvoie true si le test choisi n'est pas significatif */ /* function that returns true if the chosen test is not significant */
int (*not_significant_test)();
/* fonction qui calcule le score a partir d'une table de score */ /* function that calculates the score from a score table */
SCORE_TYPE (*compute_score_from_table)();
/* une fonction qui recherche un test pour splitter le noeud */ /* a function that looks for a test to split the node */
void (*find_a_split)(int *ls_vector, int start, int end);
/* une fonction qui recherche un seuil pour un attribut numerique */ /* a function that looks for a threshold for a numeric attribute */
void (*find_a_threshold_num)(int att, int *ls_vector, int start, int end);
/* une fonction qui recherche un seuil pour un attribut symbolique */ /* a function that looks for a threshold for a symbolic attribute */
...
/* DEFINITION DE LA "TABLE" POUR STOCKER LES ARBRES */ /* definition of the "table" for storing trees */
...
union threshold_type *threshold=NULL; /* le seuil associe a un noeud (sur 64 BITS pour reprÈsenter plus de valeurs 
				                      * symboliques */ /* the threshold associated with a node (out of 64 BITS to represent more symbolic values */
int *left_successor=NULL; /* la position RELATIVE du successeur par rapport */ /* the relative position of the successor in relation to */
int *right_successor=NULL; /* a ce noeud */ /* to this node */
int *tested_attribute=NULL; /* l'attribut teste en ce noeud */ /* the attribute tests in this node */
int *prediction=NULL; /* l'index de la prediction associe au noeud */ /* the index of the prediction associated with the node */
float *node_weight=NULL; /* un poids pour le test en dual perturb and combine */ /* a weight for the dual perturb and combine test */
/* Pour missing values: la taille de la feuille en nb d'elements */ /* for missing values: the size of the sheet in the nb of elements */
...
/* table des valeurs */ /* table of values */
...
/* parametres generaux pour la construction */ /* general parameters for construction */

int goal; /* goal_classification ou goal_regression */ /* classification goal or regression goal */

int *attribute_vector=NULL; /* indice des attributs */ /* attribute index */
int nb_attributes;

int *current_learning_set=NULL; /* l'ensemble d'apprentissage */ /* learning set */
...
int global_learning_set_size=0; /* taille des table object_mapping et object_weight: peut Ítre plus grand
				   que current_learning_set_size si certains objets sont de poids nuls*/ /* size of the object mapping and object_weight tables: can be larger than if some objects are of zero weight */

int min_node_size; /* nombre min d'objets dans un noeud */ /* min number of objects in a node */

/* parametres du tri: *
 * par defaut, on trie chaque fois localement */ /* sort parameters: by default, we sort each time locally */
...
/* methodes d'ensemble */ /* set methods */
...
/* parametres generaux pour la classification */ /* general parameters for classification */

int nb_classes; /* nombre de classes pour la classification */ /* number of classes for classification */

/* parametres generaux pour la regression */ /* general paramaters for regression */

/* ... */

/* pour le calcul du score */ /* to calculate the score */
 ...
 */ /* we could use table_score and table_score_symb but I prefer to use dedicated tables so that MAX_GOAL_MULTIREGR is independent of MAX_NUMBER_OF_NODES */
...
/* parametre de stop splitting pour les arbres */ /* the stopping parameters for the trees */

float h_min;

/* variables globales pour le calcul des meilleures score */ /* global variables to calculate the best score */
...
/* variables globales pour le stop splitting criterio a posteriori (not-significant) */ /* global variables to stop splitting based on the a posteriori condition */
...
/* BUILD_ONE_TREE */
/*----------------*/
/* fonction generale de construction d'un arbre */ /* general function to construct a tree */

int build_one_tree() {
  int tree;
  int nb_tests=0;

  /* on construit un noeud et on le place sur le stack */ /* we build a node and place it on the stack */
  index_nodes++;
  prediction[index_nodes]=-1; /* par defaut, c'est une feuille */ /* by default its a leaf */
  tested_attribute[index_nodes]=-1; /* pas de test */ /* no test */
  ...
  /* on lance la boucle de developpement, tant qu'il y a des noeuds ouverts */ /* we launch the development loop, as long as there are open nodes */
  while (index_stack_open_nodes>=0) {
    ...
    /* resume du vecteur */ /* the vector summary */
    summarize_vector(current_learning_set, start, end);
    /* pour missing_values */ 
    node_size[node]=table_score_multiregr[0][0];

    /* condition d'arret: */ /* stop condition */
    if ((nodesize==1) || (nodesize<min_node_size) || stop_splitting_criterio()) {
      /* c'est une feuille */ /* its a leaf */
      prediction[node]=make_leaf_prediction();
      index_stack_open_nodes--;

    } else {

      /* on recherche le meilleur split */ /* we look for the best split */
      find_a_split(current_learning_set, start, end);

      /* Pas vraiment un test statistique, on vérifie juste si best_threshold_score est >=0 */ /* Not really a statistical test, we just check if best_threshold_score is> = 0 */
      if (not_significant_test()) {
	    /* c'est une feuille */ /* its a leaf */
	    prediction[node]=make_leaf_prediction();
	    index_stack_open_nodes--;
      } else {
	    /* on separe les objets sur base */ /* we separate the objects on the base */
		...
		/* on cree deux nouveaux noeuds */ /* we create two new nodes */
		...
		/* on met a jour le noeud courant */ /* we update the current node */
		...
		/* on place les nouveaux noeuds sur la pile */ /* we place the new nodes on the stack */
		/* pas de best_first, on les met betement sur la pile */ /* no best_first, we stupidly put them on the stack */
...
int separate_ls_vector_local(int best_attribute, union threshold_type best_threshold, int *ls_vector, int start, int end) {
  ...
  while (start!=end) {
    ...
    if (start!=end) { /* on inverse les deux */ /* we reverse the two */
      int temp;
  ...
  /* ici, on a start=end, on renvoie la borne */ /* here, we have start = end, we return the bound */
...
/* fonction de tri (from the numerical recipes in C) */ /* sort function (from the numerical recipes in C) */
...
void find_a_threshold(int att, int *ls_vector, int start, int end) {
  /* simple dispatch sur le type de l'attribut */ /* simple dispatch on the type of the attribute */
  if (NUMERICALp(att)) {
...
/* ARBRES CLASSIQUES */ /* classic trees */
/* ----------------- */

/* GENERAL */

/* find_the_best_split */
/* recherche le meilleur split sur tous les attributs */ /* search for the best split on all attributes */
...
void find_the_best_split(int *ls_vector, int start, int end) {
  ...
  for(i=0; i<nb_attributes; i++) { /* on considere tous les attributs */ /* we consider all the attributes */
...
void find_a_split_at_random_et(int *ls_vector, int start, int end) {
  ...
  do {
    ...
    if (current_threshold_score<0.0)
      /* l'attribut etait constant, ca ne compte pas */ /* the attribute was constant, it does not count */
      nb_try--; 
 ...
 */ /* implementation of the Breiman variant for random trees
  * we select a certain number of attributes randomly in the set
  * candidate attributes for which we are looking for the best splits */
...
void find_the_best_split_among_k(int *ls_vector, int start, int end) {
  ...
  for (i=0; (i<rf_k)&&(remaining_att!=0) ; i++) {
    ...
    /* swap les attributs dans la liste */ /* swap the attributes in the list */
    remaining_att--;
    if (remaining_att!=0) {
      temp=attribute_vector[random_att_pos];
      attribute_vector[random_att_pos]=attribute_vector[remaining_att];
...
/******************************************************************************/
/* FONCTIONS UTILITAIRES DIVERSES */ /* miscellaneous utility functions */

/* valable uniquement si max_val est faible par rapport a rand_max */ /* valid only if max_val is low compared to rand_max */
/* sinon, il vaut mieux utiliser get_random_long_long */ /* otherwise, it's better to use get_random_long_long */
...
/* accede a la table de valeurs */ /* access the table of values */
...
/* UTILISATION DES ARBRES */ /* use of trees */

/****************************************
 * propagation d'un objet dans un arbre *
 ****************************************/

/* application d'un arbre a un objet */ /* applying a tree to an object */
...
/* efface tous les arbres */ /* clear all the trees */

void clean_all_trees() {
  /* efface toutes les tables */ /* clear all the tables */
  index_nodes=-1;
...
/* alloue toutes les tables de donnÈes de maniËre dynamique */ /* allocate all data tables dynamically */
...
int allocate_tree_tables(int nb_of_nodes, int nb_of_leaves, int nb_pred, int tsp) {
  ...
  /* ne sert a rien si multiregr_savepred est a 1 */ /* no use if multiregr_savepred is at 1 */
  ...
  if (nb_pred>0) {
    ...
    /* allocation de la table de score (a ne pas faire si multiregr) */ /* allocation of the score table (not to be done if multiregr) */
    if (tsp==1) {
...
/* fonction de test par defaut */ /* function of test by default */

float *(*get_tree_prediction_vector)(int tree, int obj)=get_tree_prediction_vector_classical;

/* pour changer la fonction de test */ /* to change the function of the test */
...
/*********************/
/* ENSEMBLE D'ARBRES */ /* set of trees */
/*********************/

/* description de l'ensemble d'arbres courant */ /* description of the current set of trees */
...
/* DiffÈrentes variantes pour la creation du LS */ /* Different variants for the creation of the LS */
...
float make_ls_vector_bagging(int tree) {
  ...
  /* on remet les poids a zero */ /* we reset the weights to zero */
  for (i=0; i<global_learning_set_size; i++)
    object_weight[i]=0.0;

  /* on incremente les poids d'objets tires au hasard */ /* we increment the weights of randomly selected objects */
  for (i=0; i<global_learning_set_size; i++) {
    int rn=get_random_integer(global_learning_set_size);
    object_weight[rn]+=1.0;
  }
  
  /* on construit le ls en prenant les objets de poids non nuls */ /* we build the ls by taking the objects of non-zero weight */
...
/* diffÈrentes variantes pour le test des arbres */ /* different variants for the tree test */
...
/* pour sauvegarder les ls */ /* to safeguard the ls */
...
/* Construction de l'ensemble d'arbres */ /* Construction of the set of trees */ 
...
float build_one_tree_ensemble(int *ts_vector, int length_ts_vector) {
  ...
  /* on vide tout */ /* we empty everything */
  clean_all_trees();
  current_nb_of_ensemble_terms=0;
  current_sum_weight=0.0;

  /* verification de la memoire */ /* memory check */
  nbn=(2*global_learning_set_size-1);

  if (!store_ensemble) { /* ensemble non stocke */ /* set not stored */
    if (size_current_tree_table < nbn) {
      /*memoire trop faible pour construire un arbre*/ /* memory too low to build a tree */
      return -1.0;
    }
  } else {/* ensemble stocke en memoire totalement */ /* set stored in memory totally */
    
    if (size_current_tree_table <(number_of_ensemble_terms*nbn)) {
      /*memoire trop faible pour construire l'ensemble d'arbres*/ /* memory too low to build the set of trees */ 
      return -1.0;
    }
  ...
  /* allocation de la matrice de test si necessaire */ /* allocation of the test matrix if necessary */
  /* de la taille (length_ts_vector*nb_pred) */ /* the size */
  
  /* initialisation de l'ensemble d'apprentissage */ /* initialization of the training set */
  make_ls_vector(-1);
  
  /* boucle de construction */ /* construction loop */
  for (t=0; t<number_of_ensemble_terms; t++) {
    int current_tree;

    /* si on le demande, on sauve les LS */ /* if we ask to save the LS */
     ...
     */ /* for compute_node_subset_current_ensemble. Ideally, we should implement it.* also for boosting taking weight into account. It would make it possible to do calculations * of variable importance more precise */
    if (save_ensemble_ls) {
    ...
    /* construction du modele */ /* model construction */
    current_tree=build_one_tree();

    sum_complexity+=index_nodes-current_tree+1;
    current_weight=make_ls_vector(current_tree);

    /* stockage du modele si on ne teste pas tout de suite */ /* storage of the model if we do not test immediately */
    if (save_ensemble_while_growing) {
    ...
    if (current_weight==0.0) {
      /* on arrete, le modele precedent est trop mauvais */ /* we stop, the previous model is too bad */
      t=number_of_ensemble_terms;
    } else if (current_weight<0) {
      /* on arrete, le dernier modele est parfait. Il devient le seul
	 modele */ /* we stop, the latest model is perfect. it becomes the only model */
      t=number_of_ensemble_terms;
      if (store_ensemble) {
...
/* pour virer l'ensemble */ /* to turn the set */
 ...
 ****************************************/ /* saving and loading mode les */

/* calcule le nombre de noeud dans un arbre */ /* calculate the number of nodes in a tree */
...
/* Ecrit un arbre dans le fichier (version propre a l'algorithme de construction) */ /* Write a tree in the file (version specific to the construction algorithm) */

void write_one_tree(int tree, FILE *fp) {
  int nb_nodes=get_tree_nb_nodes(tree);
  int current_node;

  /* on ecrit le nombre de noeuds */ /* we write the number of nodes */
  fwrite(&nb_nodes, sizeof(int), 1, fp);

  /* on ecrit l'info sur les noeuds */ /* we write the info on the nodes */
  for (current_node=tree; current_node<tree+nb_nodes; current_node++) {
    int pred;
    /* on ecrit les info sur ce noeud dans le fichier */ /* we write the info on this node in the file */
    /* on ecrit 0 ou 1 selon la valeur de prediction[current_node] */ /* we write 0 or 1 according to the prediction value [current_node] */
    /* deux cas: */ /* two cases */
    if (left_successor[current_node]!=-1) {
      pred=-1;
      ...
      /* normalement, on n'a pas besoin de cette valeur */ /* normally we don't need this value */
      fwrite(&right_successor[current_node], sizeof(int), 1, fp);
    } else {
      pred=1;
      fwrite(&pred, sizeof(int), 1, fp);
      /* on ecrit les valeurs de predictions */ /* we write the prediction values */
...
float (*getobjy_multiregr_learn)(int obj, int i); /* permet de separer la table des entrees de la table des sorties */ /* allows to separate the input table from the output table */
...
/* calcule les stats sur les sorties */ /* calculate the stats on the exits */
...
/* fonction de repropagation des objets dans l'arbre */ /* repropagation function of objects in the tree */

/* SCORE_TYPE v_tot; */

/* le score est simplement la somme des réductions de variance */ /* the score is simply the sum of the variance reductions */
...
SCORE_TYPE compute_multiregr_score_from_table() {
  ...
  return (info/v_tot); /* le score est compris entre 0 et 1. La
			  normalisation n'est pas nécessaire */ /* the score is between 0 and 1. Normalization is not necessary */
}

/* stop splitting criterio pour la distance */ /* stop splitting criterio for the distance */

/* SCORE_TYPE v_min=0.0; */
/* verifier ce calcul) */ /* verify this calculation */
 ...
 */ /* nothing is saved for the moment, the prediction will be calculated at the time of the test. It avoids retaining a large vector
  * for each sheet (but it increases the calculation times for the test) */
...
/* recherche le meilleur seuil pour la distance */ /* find the best threshold for the distance */

/* attribut numérique */ /* numerical attribute */
...
void find_the_best_threshold_multiregr(int att, int *ls_vector, int start, int end) {
  ...
  /* initialisation de la table */ /* table initialisation */
  ...
  /* on trie l'ensemble selon l'attribut */ /* we sort the set according to the attribute */
  sort_ls_vector(ls_vector, start, end, att);

  /* on parcourt toutes les valeurs de seuils possibles */ /* we go through all the possible threshold values */
  ...
  for(st=start; st<end; st++) {
    ...
    if ((new_val=getattval(object_mapping[ls_vector[st+1]],att))!=old_val) { /* un nouveau seuil a considerer */ /* a new threshold to consider */
	...
	if (old_val>=best_threshold) /* problem d'arrondi */ /* rounding problem */
...
/* attribut symbolique */ /* symbolic attribute */
...
/* ATTENTION version super inefficace et naive */ /* caution: super inefficient and naive version */
...
void find_the_best_threshold_symb_multiregr(int att, int *ls_vector, int start, int end) {
  ...
  /* on precalcule la table avec tous les frequences pour toutes les classes */ /* we pre-calculate the table with all the frequencies for all the classes */
...
void find_a_threshold_at_random_multiregr(int att, int *ls_vector, int start, int end) {
  ...
  /* calcule les stats sur l'attribut */ /* calculate the stats on the attribute */
  ...
  if (min==max) { /* toutes les valeurs sont egales */ /* all the values are equal */
    return;
  }
  
  /* tirage du seuil (uniformément entre min et max) */ /* threshold pull (uniformly between min and max) */
  current_threshold.f=max-(max-min)*get_random_float();
  
  /* calcul du score */ /* calculate score */
  table_score_multiregr[1][0]=0.0;
  for (i=0; i<nb_goal_multiregr; i++) {
    table_score_multiregr[1][2*i+1]=0.0;
    table_score_multiregr[1][2*i+2]=0.0;
...
/***********************************************/
/* IMPORTANCE DES VARIABLES (version multiple) */ /* importance of variables */
/***********************************************/

/* code separé par rapport aux arbres classiques mais les deux pourraient être combinés sans problème */ /* separate code compared to conventional trees but the two could be combined without problem */
/* pour le moment, ts_vector contient des index dans object_mapping (de 0 à N)
 */ /* for the moment, ts_vector contains indexes in object_mapping (from 0 to N) */

/* il faudrait l'allouer de manière dynamique */ /* it should be allocated in a way that is dynamic */
...
void get_vi_multiregr_separate(int *ts_vector, int start, int end, int borne, SCORE_TYPE *vi) {
  ...
  /* calcule v_tot en fait */ /* calculate v_tot in fact */
  
  if ((start>=borne)||(borne>end)) {
    /* tous les objets sont a droite ou a gauche -> vi=0 
     * (ne peut arriver qu'a cause d'erreur d'arrondi ?)
     */ /* all objects are right or left -> vi = 0
      * (can only happen because of rounding error?) */
    for (i=0; i<nb_goal_multiregr; i++) {
      vi[i]=0;
    }
    return;
   ...
   * objet par objet */ /* fill the table_score, there is surely a more efficient way than to do it
    * object by object */
...
int compute_one_tree_variable_importance_multiregr_separate(int tree, int *ts_vector, int length_ts_vector, float weight,
  ...
  if (vi==NULL) {
    /*Impossible d'allouer de la memoire*/ /* impossible to allocate memory */
    return 0;
  } else {
	  ...
	  while (index_stack_open_nodes>=0) {
	    ...
	    if ((left_successor[node]==-1)||(node_size==1)) {
	      ...
	      /* calcul de l'importance (seulement si borne OK) */ /* calculation of importance (only if terminal OK) */
	      get_vi_multiregr_separate(ts_vector, start, end, borne, vi);

	      /* mis a jour du vecteur */ /* updated vector */
	      for (i=0; i<nb_goal_multiregr; i++)
		...
		if (check_test(tested_attribute[node],getattval(obj,tested_attribute[node]),threshold[node])) {
		  /* on met le gauche */ /* we put the left */
		  index_stack_open_nodes++;
		  stack_open_nodes[index_stack_open_nodes][0]=node+left_successor[node];
		  stack_open_nodes[index_stack_open_nodes][1]=start;
		  stack_open_nodes[index_stack_open_nodes][2]=borne-1;
		} else {
		  /* on met le droit */ /* we put the right */
		  index_stack_open_nodes++;
		  stack_open_nodes[index_stack_open_nodes][0]=node+right_successor[node];
		  stack_open_nodes[index_stack_open_nodes][1]=borne;
		  stack_open_nodes[index_stack_open_nodes][2]=end;
...
int compute_ltrees_variable_importance_multiregr_separate(SCORE_TYPE *attribute_importance, int obj) {
   ...
   */ /* if obj is -1 -> classical calculation
    * otherwise, we only browse the branch of the tree through which the object passes
    * useful especially for a set of trees. Allows you to find a kind of branch
    * tests (fuzzy or prototype) corresponding to this object. */
  ...
  /* remplit le vecteur */ /* fill the vector */
  ...
  /* boucle sur les arbres */ /* tree loop */
  if ((current_nb_of_ensemble_terms==0) && (index_nodes>=0)) {
    /* il y a un arbre mais pas d'ensemble. On calcule l'importance de cet arbre uniquement */ /* there is a tree but no set. We calculate the importance of this tree only */
